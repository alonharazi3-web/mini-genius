...
const Admin={
  async render(){
    ...
    +'<button class="btn btn-outline" style="width:100%;margin-bottom:8px;" onclick="Admin.importData()">📥 ייבוא נתונים</button>'
    +'<input type="file" id="importFile" accept=".json" style="display:none" onchange="Admin.processImport(this)">'
    +'</div>';

    // v3.0: Factory reset
    html+='<div class="admin-section" style="border:2px solid var(--danger);"><h3 style="color:var(--danger);">⚠️ אזור מסוכן</h3>'
    +'<div class="info-box" style="background:#fef2f2;border-color:#fecaca;color:#991b1b;">איפוס מלא ימחק את כל הנתונים: מועמדים, הגדרות, היסטוריה, מחזורי גיוס, קבצים — הכל.</div>'
    +'<button class="btn btn-danger" style="width:100%;" onclick="Admin.factoryReset()">🗑 איפוס להגדרות יצרן</button>'
    +'</div></div>';
  ...
  // v3.0: Factory reset — delete ALL data
  async factoryReset(){
    if(!confirm('⚠️ פעולה זו תמחק את כל הנתונים!\n\nמועמדים, הגדרות, היסטוריה, מחזורי גיוס, קבצים — הכל יימחק לצמיתות.\n\nהאם להמשיך?'))return;
    if(!confirm('אישור סופי — האם אתה בטוח?\nמומלץ לייצא גיבוי לפני מחיקה.'))return;
    try{
      _dbg('FACTORY RESET — deleting all data');
      var stores=['candidates','settings','jobs','tasks','daylog','files'];
      for(var i=0;i<stores.length;i++){
        var all=await DB.getAll(stores[i]);
        for(var j=0;j<all.length;j++){
          var key=all[j].id||all[j].key;
          if(key)await DB.del(stores[i],key);
        }
      }
      _dbg('All stores cleared');
      // Re-init defaults
      await DB.initDefaults();
      App.settings=await DB.getAllSettings();
      App.currentJob=null;
      var jobs=await DB.getAllJobs();
      if(jobs.length){App.currentJob=jobs[0].id;}
      App.renderJobLabel();App.renderTabs();App.updateBadges();
      Utils.toast('כל הנתונים נמחקו — איפוס בוצע','success');
      Admin.render();
    }catch(e){
      _dbg('Factory reset err: '+e);
      Utils.toast('שגיאה באיפוס','danger');
    }
  },

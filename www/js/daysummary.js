...
const DaySummary={
  ...
  async prepareCloseDay(){
    ...
    try{
      ...
      // 3. Word questionnaires — ONLY candidates graded TODAY
      var allCands=await DB.getAllCandidates();
      var withQ=allCands.filter(function(c){
        if(!c.stage2_q_grade||c.jobId!==App.currentJob)return false;
        var gradeDate=c.stage2_completedAt||c.updatedAt||'';
        return gradeDate.startsWith(today);
      });
  ...
  // Pre-fill email via mailto as secondary action
  async sendSelected(){
    ...
    try{
      ...
      if(fileUrls.length&&window.plugins&&window.plugins.socialsharing){
        // shareWithOptions — stable, doesn't crash
  ...
  _buildPreviewHtml:function(c){
    ...
    var sections=[
      {title:'פרטים אישיים',fields:[
        ...
        {label:'ילדים',value:q('children')},{label:'מוכנות לרילוקציה',value:q('relocation')},
        {label:'משרה מלאה',value:q('fullTime')},{label:'רשיון נהיגה',value:q('license')},
        {label:'רשיון C',value:q('licenseC')},{label:'ימי התלבטות',value:q('hesitationDays')}
      ]},
      {title:'מצב רפואי',fields:[
        ...
        {label:'סממנים',value:q('tattoos')},
        {label:'זמינות',value:q('availability'),detail:q('availabilityDetail')}
      ]},
      {title:'רקע והשכלה',fields:[
        {label:'עיסוק נוכחי',value:q('currentJob')},{label:'סיפור חיים',value:q('lifeStory')},
        ...
        {label:'מכינה/ישיבה',value:q('mechina'),detail:q('mechinaDetail')}
      ]},
      {title:'שירות צבאי ותעסוקה',fields:[
        ...
        {label:'לאחר צבא',value:q('postArmy')}
      ]},
      {title:'שאלות אינטימיות',fields:[
        ...
        {label:'משמעת צבאית',value:q('intimateMilitary'),detail:q('intimateMilitaryDetail')}
      ]}
    ];
    return Utils.exportQuestionnaireAsWord(Utils.exportName(c),sections,q('grade'),q('result'),q('notes'),q('rejectionReason'));
  },
  ...
  _buildDocx:function(c){
    ...
    return DocxBuilder.build(Utils.exportName(c),sections,q('grade'),q('result'),q('notes'),q('rejectionReason'));
  },
  ...
  async _generateDashboardHtml(){
    ...
    for(var i=1;i<=7;i++){
      var st=Utils.getStage(i);var d=allData[i];
      if(d.active.length){
        html+='<h2>'+st.icon+' '+st.name+' — פעילים</h2><table><tr><th>שם</th><th>תחנה</th><th>סטטוס</th></tr>';
        d.active.forEach(function(c){
          html+='<tr><td>'+Utils.escHtml(Utils.exportName(c))+'</td><td>'+Utils.getStageName(c.stage)+'</td><td>'+Utils.STATUSES[c.status]+'</td></tr>';
        });
        html+='</table>';
      }
    }

...
const Dashboard={
  ...
  async showNames(stageId,type){
    ...
    list.forEach(function(c){
      ...
      +Utils.escHtml(Utils.displayName(c))+' <span style="color:var(--text-light);font-size:.78rem;">'
  ...
  async exportReport(){
    ...
    function nameList(id,arr){
      ...
      arr.forEach(function(c){h+='<div class="li">'+Utils.escHtml(Utils.exportName(c))+' <span class="ph">'+Utils.getStageName(c.stage)+'</span></div>';});
